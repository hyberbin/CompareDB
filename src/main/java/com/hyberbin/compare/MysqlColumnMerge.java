 ...
 * @author Hyberbin
 */
public class MysqlColumnMerge implements IColumnMerge {
    ...
    public MysqlColumnMerge(String table, String createTableSql) {
        String[] sql = createTableSql.split("\n");
        for (String clo : sql) {
            if (clo.trim().startsWith("`")) {
                int last_quot=clo.endsWith(",")?clo.length()-1:clo.length();
                columnMap.put(getCloName(clo), clo.substring(0, last_quot));
            }
        }
        this.table = table;
